...
//                // print(dictScanningData)
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if APIstruct.isApiError{
            GlobalMethods.showAlertView(APIstruct.apiError, with: self)
            APIstruct.isApiError = false
            APIstruct.apiError = ""
        }
    ...
    func setData(){
        ...
        for index in 0..<26 + appDocumentImage.count{
            ...
            case 5:
                if(givenNames != "") {
                    let stringWithoutSpaces = givenNames.replacingOccurrences(of: "<", with: " ")
                    dict = [KEY_VALUE: stringWithoutSpaces,KEY_TITLE:"First Name"] as [String : AnyObject]
                    arrDocumentData.append(dict)
                }
                break
            case 6:
...
//                    }
                ...
                if !arrDataBackKey.isEmpty{
                    ...
                    if objDatakey.contains("Sign") || objDatakey.contains("SIGN") || objDatakey.contains("Signature"){
                        if let decodedData = Data(base64Encoded: objDataValue, options: .ignoreUnknownCharacters) {
                            let image = UIImage(data: decodedData)
                            cell.imageViewSignHeight.constant = 51
                            cell.imageViewSign.image = image
...
//                ProgressHUD.dismiss()
    ...
    func date(to dateStr: String?) -> String? {
        ...
        if let date = date {
            ...
            let str = reverse(dateStr!)
            let sstr = arrangeDateFormat(strPhone: str)
            return sstr
        }
        return nil
    }
    ...
    var characters = Array(str)
        ...
        characters.swapAt(2, 3)
        characters.swapAt(4, 5)
        let string = String(characters)
     return string
    }
    
    func arrangeDateFormat(strPhone : String)-> String {
